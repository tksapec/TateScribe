...
`OpenXmlDocumentExporter` supports the legacy export contract and the structured multi-ruby contract. Structured ruby is written as schema-valid WordprocessingML, while the existing paragraph styles and chapter page-break option remain intact. `DocxRubyOptions` defaults the Word ruby offset to 3pt and validates the inclusive 0 through 20 range. `WordRubyMetrics` calculates a provisional raise as ruby half-points plus twice that offset; an offset change requires a DOCX re-export. The compare script normalizes direct-export XML for diagnosis, but B/C comparison and manual Word visual verification require Word-saved reference files and remain outside automated verification.

`RubyReviewWindow` uses the existing selection service for Ctrl/Shift extended selection, the Ctrl+Enter confirmation shortcut, and explicit Confirm/Reject buttons; rejection is button-only rather than a Delete-key action. Its summary exposes the selected count. Image and text bulk confirmation are intentionally separate and each dialog reports examined, newly confirmed, already confirmed, wrong-source, excluded, and exclusion-reason categories. These UI/export changes use existing persisted state: no SQLite schema migration and no Release ZIP are produced.
