...
5. Manual edits are separate immutable revisions. A regenerated OCR layer may replace only machine-owned evidence.
6. `DocxExportService` consumes the resolved text view and produces Open XML elements, adding ruby only when parent/ruby alignment is confirmed.
